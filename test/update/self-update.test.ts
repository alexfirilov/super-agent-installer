...
describe('selfUpdate', () => {
  it('reports current when not newer', async () => { expect(await selfUpdate(makeTestCtx(), '0.2.0', { fetch: f('0.2.0'), execPath: '/opt/sai/super-agent-installer', platform: 'linux', arch: 'x64' })).toMatchObject({ updated: false, ok: true, message: expect.stringMatching(/current/) }); });
  it('downloads, verifies and replaces the executable', async () => {
    ...
    expect(r.updated).toBe(true); expect(r.ok).toBe(true); expect(readFileSync(exe, 'utf8')).toBe('new-binary'); expect(existsSync(`${exe}.new`)).toBe(false);
  });
  it('refuses on checksum mismatch', async () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), 'sai-')); const exe = join(dir, 'super-agent-installer'); writeFileSync(exe, 'old');
    const r = await selfUpdate(makeTestCtx(), '0.1.0', { fetch: bad, execPath: exe, platform: 'linux', arch: 'x64' }); expect(r.updated).toBe(false); expect(r.ok).toBe(false); expect(r.message).toMatch(/checksum/); expect(readFileSync(exe, 'utf8')).toBe('old');
  });
  it('does nothing when running from source', async () => { const r = await selfUpdate(makeTestCtx(), '0.1.0', { fetch: f('9.9.9'), execPath: '/usr/bin/bun', platform: 'linux', arch: 'x64' }); expect(r.message).toMatch(/source/); expect(r.ok).toBe(true); });
  it('cleans up the temp file and reports failure when replacing the executable fails', async () => {
    const dir = mkdtempSync(join(tmpdir(), 'sai-')); const exe = join(dir, 'super-agent-installer'); writeFileSync(exe, 'old');
    const rename = async () => { throw new Error('rename EACCES'); };
