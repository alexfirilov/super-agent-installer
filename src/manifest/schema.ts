...
const skillSpec = z.object({ kind: z.literal('skill'), repo: z.string(), skills: z.union([z.array(z.string()).min(1), z.literal('*')]), targets: z.array(z.enum(['claude-code', 'codex'])).min(1), postInstall: z.array(z.array(z.string())).optional() });
const mcpSpec = z.object({ kind: z.literal('mcp'), target: z.enum(['claude', 'codex']), name: z.string(), transport: z.enum(['http', 'stdio']), url: z.string().optional(), bearerEnv: z.string().optional(), command: z.string().optional(), args: z.array(z.string()).optional(), env: z.record(z.string(), z.string()).optional(), secretEnv: z.array(z.string()).optional(), extra: z.record(z.string(), z.unknown()).optional(), oauth: z.object({ clientId: z.string(), callbackPort: z.number() }).optional(), postInstall: z.array(z.array(z.string())).optional() })
...
const settingSpec = z.object({ kind: z.literal('setting'), target: z.enum(['claude', 'codex']), claudeSettings: z.record(z.string(), z.unknown()).optional(), claudeJsonSeed: z.record(z.string(), z.unknown()).optional(), codexToml: z.record(z.string(), z.unknown()).optional(), codexFeatures: z.record(z.string(), z.boolean()).optional(), windowsGitConfig: z.record(z.string(), z.string()).optional() });
