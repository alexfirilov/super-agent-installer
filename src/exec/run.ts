...
export function createRunner(o: { dryRun: boolean; log: Logger; env?: Record<string, string | undefined> }): Runner {
  return async (argv: string[], opts: RunOptions = {}): Promise<RunResult> => {
    ...
    return await new Promise<RunResult>((resolve, reject) => {
      const child = spawn(cmd, args, { cwd: opts.cwd, env: { ...process.env, ...o.env, ...opts.env }, shell: opts.shell ?? false, stdio: interactive ? 'inherit' : ['pipe', 'pipe', 'pipe'], windowsHide: true });
      let stdout = '', stderr = '', done = false, stopped = false;
      // interactive mode captures nothing, so there is nothing for stopOnOutput to match: it never fires.
      const check = () => { if (!interactive && !stopped && !done && opts.stopOnOutput?.test(stdout + stderr)) { stopped = true; o.log.debug(`stopping ${shown}: output matched ${opts.stopOnOutput}`); child.kill(); setTimeout(() => { if (!done) child.kill('SIGKILL'); }, 2000).unref(); } };
      ...
      if (!interactive) { child.stdout?.on('data', (d) => { stdout += d; check(); }); child.stderr?.on('data', (d) => { stderr += d; check(); }); }
      child.on('error', (e) => {
      ...
      if (!interactive) { if (opts.input !== undefined) child.stdin?.write(opts.input); child.stdin?.end(); }
    });
  };
}
