...
/** Directories the vendor installers drop binaries into during a run (Claude/Codex native installs, fnm's Node, npm --prefix ~/.local). */
export function toolDirs(host: HostInfo, env: Record<string, string | undefined>): string[] {
  ...
  return [join(h, '.local', 'bin'), fnm, join(fnm, 'aliases', 'default', 'bin'), '/usr/local/bin'];
}
