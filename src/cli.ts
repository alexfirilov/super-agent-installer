...
export interface CliArgs { command: Command; profile?: ProfileName; only?: string[]; skip?: string[]; yes: boolean; dryRun: boolean; json: boolean; noAudit: boolean; noSelfUpdate: boolean; noLogin: boolean; noPersistSecrets: boolean; fromState: boolean; channel?: Channel; verbose: boolean; manifest?: string; logFile?: string; elevate: boolean; ids: string[]; error?: string }
...
Flags: --yes/-y  --dry-run  --json  --no-audit  --no-login  --no-persist-secrets  --channel latest|stable  --from-state  --manifest <path>  --log-file <path>  --elevate  --verbose/-v  --version  --help/-h
  --elevate  Windows only: allow machine-scope winget installs (UAC prompt). Without it, installs stay per-user (scoop / winget --scope user / npm / script).`;
export function parseCli(argv: string[]): CliArgs {
  const { values, positionals } = parseArgs({ args: argv, allowPositionals: true, strict: true, options: { profile: { type: 'string' }, only: { type: 'string' }, skip: { type: 'string' }, yes: { type: 'boolean', short: 'y', default: false }, 'dry-run': { type: 'boolean', default: false }, json: { type: 'boolean', default: false }, 'no-audit': { type: 'boolean', default: false }, 'no-self-update': { type: 'boolean', default: false }, 'no-login': { type: 'boolean', default: false }, 'no-persist-secrets': { type: 'boolean', default: false }, 'from-state': { type: 'boolean', default: false }, channel: { type: 'string' }, verbose: { type: 'boolean', short: 'v', default: false }, manifest: { type: 'string' }, 'log-file': { type: 'string' }, elevate: { type: 'boolean', default: false }, version: { type: 'boolean', default: false }, help: { type: 'boolean', short: 'h', default: false } } });
...
export async function main(argv = process.argv.slice(2)): Promise<number> {
  ...
  const ctx = await createCtx({ dryRun: a.dryRun, yes: a.yes, noAudit: a.noAudit, channel: a.channel, verbose: a.verbose, json: a.json, manifestPath: a.manifest, logFile: a.logFile, elevate: a.elevate });
