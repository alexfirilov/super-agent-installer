...
export interface CliArgs { command: Command; profile?: ProfileName; only?: string[]; skip?: string[]; yes: boolean; dryRun: boolean; json: boolean; noAudit: boolean; noSelfUpdate: boolean; noLogin: boolean; fromState: boolean; channel?: Channel; verbose: boolean; manifest?: string; logFile?: string; ids: string[]; error?: string }
...
export function parseCli(argv: string[]): CliArgs {
  ...
  return { command, profile: values.profile as ProfileName | undefined, only: list(values.only), skip: list(values.skip), yes: values.yes, dryRun: values['dry-run'], json: values.json, noAudit: values['no-audit'], noSelfUpdate: values['no-self-update'], noLogin: values['no-login'], fromState: values['from-state'], channel: values.channel as Channel | undefined, verbose: values.verbose, manifest: values.manifest, logFile: values['log-file'], ids, error };
}
export async function main(argv = process.argv.slice(2)): Promise<number> {
  ...
  switch (a.command) {
    case 'install': {
      ...
      if (interactive) { const state = await readState(ctx.paths.stateFile); const pick = await runPicker(ctx, { state }); if (!pick) { console.log('cancelled'); return 0; } ctx.yes = false; return runInstall(ctx, { picked: pick.picked, profile: pick.profile === 'saved' ? undefined : pick.profile, json: a.json, installerVersion: VERSION, noLogin: a.noLogin }); }
      return runInstall(ctx, { profile: a.profile, only: a.only, skip: a.skip, fromState: a.fromState, json: a.json, installerVersion: VERSION, noLogin: a.noLogin });
    }
