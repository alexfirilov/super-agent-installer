...
export function createLogger(o: LoggerOptions): Logger & { lines: string[] } {
  const lines: string[] = [];
  if (o.file) { mkdirSync(dirname(o.file), { recursive: true }); if (process.platform !== 'win32') { try { chmodSync(o.file, 0o600); } catch { /* created on first write with mode 0600 */ } } }
  const emit = (level: string, msg: string, show: boolean) => {
    const line = `${new Date().toISOString()} ${level.padEnd(5)} ${msg}`;
    if (o.file && (level !== 'DEBUG' || o.verbose)) appendFileSync(o.file, line + '\n', { mode: 0o600 });
